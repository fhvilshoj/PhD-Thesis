...
% \end{description}
% \fref{chap:fasth}, \href{https://fhvilshoj.github.com}{fhvilshoj.github.com}
...
% % % %  % % % %  % % % %  % % % % 
% New structure for Thesis:
% % % %  % % % %  % % % %  % % % % 

% The idea here is a top-down approach: 1. Introduce ECINN, as a way to use generative models, then ii) Say that when NFs are improved, so are CFs; we improve in SVD paper. iii) In the GAN literatyre, FID is typically used for evaluation, we evaluate how FID stacks up against other metrics in [metrics paper]. As FID is a valuable tool for evaluating counterfactuals, having a fast way to compute FID for small batches may help improve iteration cycles for counterfactual explanations and could potentially be used with a new gradient based method for generating counterfactuals.

\chapter{Introduction} 
% +++ 0.5 pages of over all introduction to the field of machine learning.
% Introduction.
% 1. Neural networks are Ubiqutous and millions of millions of dollars are being spent on training high-performing classifiers, generative models, etc. 
% Fundamental for such training is efficient computations, as hardware is expensive and training requires a lot of power. 
% We have studied various aspects of efficiently training generative models.
% At the time of writing, deep neural networks are practically ubiquitous in the field of machine learning research.

% +++   1 page of introducing to generative models  

% State our main hypothesis that generative models are closely related to counterfacutal generation.
% +++   1 page on motivating counterfactuals
...
Efficiently training neural networks pose a significant challenge within the field of machine learing, as neural networks are costly to train and can be found solving a variety of problems from classifying malignant tumors~\cite{cancer} to generating realistic ``fake'' data~\cite{gpt-3}. 
...
Speeding up computations potentially improves performance of the models on performance metrics like likelihood and accuracy to consequently improve explainability further downstream.
...
With the new algorithm, FID can be optimized directly as a part of a loss function, which, prior to our work, was expensive to do in terms of amounts of compute.
% Prior to this paper, the FID was an ill fit for a training loss due to high amounts of computation.
...
In this section, we first give a brief introduction to the high-level contributions of our paper, before putting it into context of related work. Successively, additional details about our algorithm are covered, and successively we present applications of the algorithm. 
...
It is, however, not straight forward how to optimize $U$ and $V$ in such representation.
...
That is, to evaluate $WX$, one must compute $2d$ sequential matrix-vector operations.
...
As a consequence, they suggest an additional algorithm based on matrix-matrix products, which is more parallel. 
...
\paragraph{Contributions.} In \paperref{pap:svd}, we introduce a hybrid algorithm FastH, which computes the exact same Householder product and obtains the fast asymptotic running time $O(d^2m)$, while requiring only $O(t + d/t)$ sequential matrix-matrix operations for an adjustable group size $t$, as compared to $O(d)$ sequential matrix-vector computations used by the sequential algorithm.
In practice, we find that FastH is 27 times faster than the sequential algorithm and six times faster than the parallel. 
...
The first examples is inverse computations. 
Inverting a $d\times d$ matrix generally takes $O(d^3)$ time.
With the SVD, we can compute the matrix inverse as follows:
\begin{equation}\label{eq:svd-inverse}
    W^{-1} = (U\Sigma V^\intercal)^{-1} = (V^\intercal)^{-1}\Sigma^{-1}U^{-1} = V \Sigma^{-1} U^\intercal.
\end{equation}
From Equation \eqref{eq:svd-inverse}, we see that it transposing $U$ and $V$, and inverting the diagonal matrix $\Sigma$ is enough.
...
After training a neural network, a natural way to compress a neural network to get a ``low rank approximation'' is thus to use only some principal components of each weight matrix.
To circumvent training large networks before compressing them, the idea has been extended in multiple ways.
...
\citet{convolution-tensor-decomposition} also propose a multi-linear tensor decomposition of convolutional filters to achieve efficient low-rank convolutional kernels.
...
For such computations, gradient computations become very ``long,'' which can result in vanishing or exploding gradients if $W$ and $\sigma$ is not norm preserving.
Orhogonal matrices are norm preserving, \ie, $\|Wx\|_2 = \|x\|_2$ for orthogonal matrix $W$, and constitute central building blocks for avoiding exploding (vanishing) gradients.
...
As described, \cite{seguential} presents both a sequential and a parallel algorithm for evaluating the Householder parametrization.
For an input $X\in\mathbb{R}^{d\times m}$, evaluating $WX$ has an $O(d^2m)$ time complexity, but is slow in practice, while the parallel algorithm is faster in practice, but has an $O(d^3)$ time complexity. 
...
It should also be mentioned that concurrent with \paperref{pap:svd}, \citet{cwy-dec} also notice the gap between the sequential and parallel algorithm and present another parallel algorithm for evaluating Householder products using a related CWY decomposition.
The algorithm has similar time complexities to FastH
...
From the observation observation that there is a gap between the slow (in practice) $O(d^2m)$ sequential algorithm and the faster parallel $O(d^3)$ algorithm from \cite{sequential}, we raised the following question.

\begin{problem}
    Given $X\in \mathbb{R}^{d\times m}$ and $v_1, ..., v_d \in \mathbb{R}^{d}$, can we evaluate $\left[ \prod_{i=1}^d I-2\frac{v_iv_i^\intercal}{\|v_i\|_2^2}\right]X = \left[\prod_{i=1}^d H_i\right]X$ with less than $O(d)$ matrix-matrix operations?
\end{problem}
...
\end{equation}
% \paragraph{Step 2.}
\item[Step 2:] From the decompositions, $G1(G2(...(G_{d/t}X)...))$ is computed sequentially by multiplying $X$ on each decomposition from right to left, \ie, $I-W_j(Y_j^\intercal X)$.
\end{description}
...
To compute the FID, on first draws real samples $r_1, r_2, ..., r_n \sim p(x)$ and fake samples $f_1, f_2, ..., f_m\sim q(x)$.
...
Two multivariate normal distributions $\mathcal{N}(\mu_r, \Sigma_r)$, $\mathcal{N}(\mu_f, \Sigma_f)$ are then formed from the statistics encodings.
...
\end{equation}
\citet{fid} demonstrate how the FID correlate well with both human judgement and increasing disturbance, \eg, adding increasing levels of noise. 

When computing the FID, only $\mu_f$ and $\Sigma_f$ changes between generative models.
...
In \paperref{pap:fastfid}, we introduce FastFID; an algorithm which computes the FID on mini-batches ($m \ll d$) in $O(d^2m + m^3)$ time, up to three orders of magnitudes faster and with at least three orders of magnitude less numerical error than the original implementation of FID.
We demonstrate how the algorithm is fast enough to be used for explicitly training GANs with FID as a loss function, resulting in better validation FID scores of generated samples. 
...
To describe how FastFID works, we first highlight how the previous algorithm computes the FID, before describing FastFID in details.
...
As mentioned, the first bottleneck can be circumvented by precomputing statistics for the real samples.
...
As an example, if we use precomputed statistics and reduce $m$ from $10 000$ to $128$, this reduces computation time from roughly $10s$ so $0.1s$ on our machine.
...
\end{lemma}
To compute the trace of the matrix square root faster, we utilize both Lemma \ref{lem:trase-eigenvalues} and Lemma \ref{lem:product-eig} to construct a matrix $M\in \mathbb{R}^{m\times m}$, for which is much smaller than $\Sigma_r\Sigma_f$ for which the eigenvalues and thus the trace is much faster to compute.
...
\begin{equation}\label{eq:fastfid}
   tr( \sqrt{\Sigma_r\Sigma_f}) = \sum_{i=1}^{d}\sqrt{\lambda_i(C_rC_r^\intercal C_fC_f^\intercal)} = \sum_{i=1}^m \sqrt{\lambda_i(C_f^\intercal C_rC_r^\intercal C_f)}
\end{equation}
...
Furthermore, there exist similar metrics based on different ``encoders'' ~\cite{fcd} for which FastFID also applies. 
 ...
 Finally, we attempt to answer determine whether using FID as a loss function yields better images.
...
As such, we believe that the presented works constitutes two important steps towards better performing generative models.
...
We devote the next chapter to presenting multiple approaches for using generative models as central components for generating explanations for need neural networks. 
...
\citet{Wachter2017} argue that for tabular data, changes should be sparse and small to be more interpretable to humans.  % TODO Got to here..
...
% Evaluating counterfactuals. 
\chapter{TODO title for evaluation paper}
\label{chap:evaluation-paper}
\paperlabel{pap:evaluation}
\chapterprecishere{\todo{\dots}}
\includepdf[pages=-10]{papers/cfscore.pdf}

% FastFID
\chapter{Fast Freché Inception Distance}
\label{chap:fastfid}
\paperlabel{pap:fastfid}
\chapterprecishere{\todo{\dots} \todo{Update to latest draft}}
