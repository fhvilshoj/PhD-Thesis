...
\title{Deep Neural Networks\\ and \\Counterfactual Explanations}
...
En øgen netværkskompleksitet medfører typisk en højere ydeevne.
...
Instead of explicitly describing an algorithm for the computer to follow, machine learning algorithms are designed to dig through tons of images to derive an algorithm that the computer can use to guess what is in the images. 
...
For example, it is not straight forward how to get counterfactual reasoning from such networks, similar to how we did with the removal of smoke and glare of the rocket image above.
...
Furthermore, a current trend indicates that larger models (more parameters) perform better~\cite{language-models-few-shot, scaling-matters}.
In turn, improving algorithms to allow even larger models to be trained within the same computational budget can positively impact performance.
Finally, we believe that as a consequence of improved performances of generative models, better methods within explainability will also emerge further downstream.

This chapter presents two papers which revolve around more efficient computations of central components for neural networks.
...
This section first gives a brief introduction to the problem that the presented algorithm solves and the high-level contributions of the paper, before putting it into context of related work. 
Successively, applications for which our algorithm improves computational efficiency are presented.
Finally, more details on the algorithm are given at the end of this section.
...
\citet{sequential} propose to parameterize each orthogonal matrix as a composition of $d$ Householder matrices $H_i$ that are stored as $d$-dimensional vector $v_i$, to circumvent this issue:
...
The parameterization makes gradient descent possible by preserving the orthogonality of $U$ and $V$ during optimizations, while the matrix product $WX = U \Sigma V^\intercal X$ can be evaluated in $O(d^2m)$ for a mini-batch of $m$ samples $X\in\mathbb{R}^{d\times m}$, similar to regular matrix multiplication. 
...
Besides the determinant computations, which we have already mentioned, we describe three additional applications for which parameterizing weight matrices by their SVDs has advantages in terms of computational speed-ups.
...
Additionally, some works utilize the nuclear norm, which simply constitutes the sum of singular values $\|W\|_\ast = \sum_{i=1}^d \Sigma_{i,i}$~\cite{nuclear-norm-regularizer, nuclear-norm-regularizer2}. 
...
As a consequence, many solutions to the problem may be of low value to end users if they are, \eg, unrealistic or convey no semantic information.
...
As we wish to combine generative models with explainability, it is therefore natural to focus on counterfactual examples in the image domain.
...
There are, \eg, many neural networks that can be fooled to predict wrong classes by adding tiny, carefully selected, and imperceptible noise to the input image (also known as adversarial attacks)~\cite{su2019one}. 
...
For tabular data, examples methods based on genetic algorithms \cite{Hashemi2020} and heuristics based on simple statistics~\cite{Gomez2020} also exist.
However, such methods are less applicable to the image domain due to the exponential increase in input possibilities and will not be discussed further. 
...
We denote counterfactual examples generated from the latent vector on the decision boundary tipping-point counterfactuals because they constitute the exact point where prediction tips from the original class to the target class.
...
In our experiments, we find that ECINN produces high-quality counterfactual explanations much faster than the iterative approaches that we compare against. 
...
In contrast, generative models have high generative capacities and produce more compelling counterfactual examples (cf. Figure \ref{fig:generative-counterfactuals-copied}).
...
We first give a high-level overview of desired properties of visual counterfactual examples before describing existing metrics that have been proposed in related work to quantify such properties in Section \ref{sec:existing-metrics}.
Afterward, we describe how we contribute with a deeper understanding of what properties existing metrics do and do not capture before introducing two new metrics that mitigate drawbacks of existing metrics in Section \ref{sec:evaluation-paper}.
...
\begin{description} 
    ...
    According to \cite{Mothilal2020}, a counterfactual close to the input can intuitively be most useful to the users.
...
This section describes some of the quantitative metrics that have been used to evaluate counterfactual methods proposed within the image domain.
...
Furthermore, we are not aware of any other work that reports this metric.
...
It may be clear to the reader that quantitatively evaluating counterfactual explanations is not a solved problem.
...
For example, adversarial attacks may get a high (good) $\operatorname{TCV}$, but convey little or no information to the end-user, as such attacks are typically not interpretable.
...
That is, when complexity increases, the difference between scores decreases.
...
We hypothesize that such a close relationship between the classifier and the generator may lead to counterfactual examples that more closely reflect the actual behavior of the classifier. 
...
The discussion in Chapter \ref{chap:evaluation} further establishes that the suite of metrics for evaluating visual counterfactual explanations is still not complete.
...
With FastH, it is possible to parameterize weights of neural networks in their SVD efficiently.
...
At the time of writing, we further have an improvement of the FastH algorithm, which maintains the asymptotic running time $O(d^2m)$, but improves the number of sequential operations from $O(d/t + t)$ to $O(d/t + \log t)$.
Informally, we can extend the $WY$ decomposition, which is central for FastH, to make it possible to compute each group product $G_j=I-W_jY_j^\intercal$ through a tree of computations, where products in each level of the tree can be computed in parallel.  
...
We believe that this approach has great potential and constitute the beginning of a new line of research. 
For example, VAEs and GANs also enable meaningful interpolations through latent spaces.
In turn, such models could perhaps be integrate with classification networks to produce more trustworthy methods for producing counterfactual examples. 
...
As diversity, sparsity and proximity are largely left unquantified, they represent open questions, which is of high importance for the field of counterfactual examples.
Using deep neural networks with strong representational power to quantify, \eg, proximity, pose a promising direction of research.
...
As hinted at, a general benchmark for counterfactual methods is also needed to ensure proper progress within the field of counterfactual examples. 
...
If such a benchmark is to be proposed, it is critical that all desired properties of counterfactual examples are captured.
